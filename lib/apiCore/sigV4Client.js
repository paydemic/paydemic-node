...
const apiGatewayUtils = require('./utils.js')
...
module.exports = function (config) {
    ...
    awsSigV4Client.accessKey = apiGatewayUtils.assertDefined(config.accessKey, 'accessKey');
    awsSigV4Client.secretKey = apiGatewayUtils.assertDefined(config.secretKey, 'secretKey');
    awsSigV4Client.sessionToken = config.sessionToken;
    awsSigV4Client.serviceName = apiGatewayUtils.assertDefined(config.serviceName, 'serviceName');
    awsSigV4Client.region = apiGatewayUtils.assertDefined(config.region, 'region');
    awsSigV4Client.endpoint = apiGatewayUtils.assertDefined(config.endpoint, 'endpoint');

    awsSigV4Client.makeRequest = function (request) {
        return new Promise(
                    (resolve, reject) => {
            var verb = apiGatewayUtils.assertDefined(request.verb, 'verb');
            var path = apiGatewayUtils.assertDefined(request.path, 'path');
            var queryParams = apiGatewayUtils.copy(request.queryParams);
            if (queryParams === undefined) {
                queryParams = {};
            }
            var headers = apiGatewayUtils.copy(request.headers);
            ...
            var body = apiGatewayUtils.copy(request.body);
            ...
            headers[HOST] = awsSigV4Client.endpoint;
